...
- **注意**：violation_details 在 is_qualified=false 时必须提供；is_qualified=true 时可为空数组 []。对规则6的每条违规必须包含 severity 和 year。
...
def maintain_sources(debug: bool = False):
    ...
    for key, src in config["sources"].items():
        ...
        if key == "sse":
            ...
            test_code = "000001"
        test_url = src["url"].format(code=test_code)
        if _check_url(test_url):
            ...
            if new_url:
                test_new_url = new_url.format(code=test_code)
...
def _classify_violation(vtype: str) -> str:
    ...
    for severity in ["high", "medium", "low"]:
        ...
        for keyword in types:
            if keyword in vtype:
                return severity
    return "medium"  # 无法归类时默认中等

def _apply_tiered_filter(is_qualified: bool, reason: str,
                         violated_rules: List[str],
                         violation_details: List[dict]) -> Tuple[bool, str, List[str]]:
    """对Rule6违规应用分级×时效过滤，超时效窗口的违规予以豁免"""
    if is_qualified:
        return True, reason, violated_rules

    if "6" not in violated_rules:
    ...
    violations = [vd for vd in (violation_details or []) if vd.get("rule") == "6"]
...
def filter_stocks(stock_list: List[str], delay: float = 2.0, debug: bool = False) -> List[str]:
    ...
    for idx, code in enumerate(normalized, 1):
        ...
        is_qualified, reason, rules = _apply_tiered_filter(model_qualified, reason, rules, violation_details)
        if is_qualified:
