...
【工作流程】  
收到公告/新闻标题列表后，你必须：
- 逐条判断该标题是否暗示可能触发规则1～10中的任意一条
- 对任何可能相关的条目，立即进行联网搜索获取完整公告或报道以核实
- 确认不存在任何违规后，方可判为“合规”
...
def _call_qwen(model: str, code: str, external_info: str, debug: bool = False):
    ...
    user_prompt = (
        ...
        "请按照系统提示中的工作流程逐条审查所有标题，并在输出前静默完成所有搜索与核实，确保没有遗漏。\n"
        "输出JSON。"
    )
    messages = [
...
if __name__ == "__main__":
    test_stocks = ["000001.sz", "000858.sz", "600519.sh", "000639.sz"]
    result = filter_stocks(test_stocks, delay=2.0, debug=True)
    print(f"\n最终合规股票池: {result}")