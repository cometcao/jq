...
小市值择时：
    周线 macd  > 0，开仓。
    周线 macd  < 0,  日线macd < 0,平仓； 日线macd > 0,开仓；


特别说明：
    程序价格参考 morningstar大神的小市值 v2.0.6程序
...
def market_time(index2):

    #先计算周线
    hData = attribute_history(index2, 130, unit='1200m', fields=('close'), skip_paused=True)
    close = hData['close']
    ...
    if wMacd[-1] > 0:
        ...
        hData = attribute_history(index2, 130, unit='240m', fields=('close'), skip_paused=True)
        close = hData['close']
        close = np.array(close, dtype='f8')
