...
def loading():
    ''' 登陆 '''
    user = use(config.trade_acc['use'])
    user.prepare(config.trade_acc['json2'])
    return user
...
def check_to_sell(context, data):
    ...
    for stock in context.portfolio.positions:
        ...
        if pos.sellable_amount > 0:
            ...
            elif (pos.price / pos.avg_cost < sl) or (df['close'][-1]/df['close'][-2]<sl):
                log.info("add %s in sell list due to stop loss" % stock)
                to_sell.append(stock)
            elif (pos.price / g.max_price[stock] < sg):
                log.info("add %s in sell list due to tailing stop loss" % stock)
                to_sell.append(stock)
            else: # tailing stop loss
                if pos.price > g.max_price[stock]: # update the max price
