...
def get_main_money_inflow_over_circulating_mcap(stock_list, 
    ...
    # circulating mcap
    cir_mcap = get_cir_mcap(stock_list, prv_date, adjust_concentrated, is_debug)
    ...
    if adjust_concentrated:
        ...
        for stock in stock_list:
            q0=query(finance.STK_SHAREHOLDER_FLOATING_TOP10).filter(
                ...
                finance.STK_SHAREHOLDER_FLOATING_TOP10.pub_date<=current_time.strftime('%Y-%m-%d')
...
def get_main_money_inflow_over_total_money_over_time(
    ...
    # circulating mcap
    cir_mcap = get_cir_mcap(stock_list, current_time, adjust_concentrated, is_debug)
    ...
    cir_mcap['money'] = pd.Series(stock_money_data) # results in real money number
    ...
    if use_cir_mcap:
        cir_mcap['mfc'] = (cir_mcap['net_amount_main']*10000)/(cir_mcap['circulating_market_cap']*(1-cir_mcap['concentrated_ratio']/100/cir_mcap['cir_total'])*100000000)/cir_mcap['sum_count']
    else:
        cir_mcap['mfc'] = (cir_mcap['net_amount_main']*10000)/cir_mcap['circulating_market_cap']/cir_mcap['sum_count']
