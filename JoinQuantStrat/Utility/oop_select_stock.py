...
class Filter_Chan_Stocks(Filter_stock_list):
    ...
    def filter(self, context, data, stock_list):
        if context.current_dt.hour < self.long_hour_start or\
            context.current_dt.minute < self.long_min_start: # we should only try to long in the afternoon
            return []
...
class Filter_common(Filter_stock_list):
    ...
    def filter(self, context, data, stock_list):
        ...
        self.log.info("选股过滤（显示前五）:\n{0}".format(join_list(["[%s]" % (show_stock(x)) for x in stock_list[:5]], ' ', 10)))
        return stock_list
