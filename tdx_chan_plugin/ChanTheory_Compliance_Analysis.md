...
> 最后更新: 2026-06-09 (极值优先端点选择)
> 当前状态: Plan A 合并法 (FeatureSeqElement) 已稳定，编译通过，64-bit 正常运行
> 本文件详细记录了从旧版删除法到 Plan A 合并法的重构过程及合规性验证

---

## 2026-06-09 修订 — 线段端点紧邻替换（in-loop）

### 问题

无缺口模式下，findXDOnFeatureSeq 按"找到即返回"模式运行，defineXD 循环翻转方向后更高同类型端点属于"反向段"特征序列，无法被替换。

### 修复

在 defineXD 循环内、端点返回后方向翻转前，扫描 working_df 中紧邻同类型笔端点。若价格更优（TOP 更高 / BOT 更低）且后方有足够元素支撑下一段 feature_seq，则替换 xd_tb。

不改 tail_start/direction，不干扰 defineXD 循环交替逻辑。

### 缠论依据

有 3 处独立改动，均合规：

1. shortcut `continue` → bounds 检查：消除连锁跳跃，确保每级都经过完整特征序列验证（"逐级判定"原则）
2. 删除冗余死代码：条件互斥无行为变化
3. in-loop 紧邻替换："线段只能被线段破坏"——在线段方向翻转之前线段处于延续状态。翻转方向意味着默认后面的发展不会出现更极端同类型端点；若出现，应在线段被"破坏"（翻转方向）前采纳。用 `j + 2 < size` 守卫确保下段 feature_seq 有足够元素，管线自有结构验证自然排除一笔成段。
